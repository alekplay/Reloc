...
- (id)initWithItems:(NSArray*)items parentView:(UIView *)p delegate:(id<BlurMenuDelegate>)d {
    ...
    if (self) {
        ...
        close.frame = CGRectMake(0, self.frame.size.height - COLLECTION_VIEW_PADDING, self.frame.size.width / 2.0, COLLECTION_VIEW_PADDING);
        ...
        about.frame = CGRectMake(close.frame.size.width, self.frame.size.height - COLLECTION_VIEW_PADDING, self.frame.size.width / 2.0, COLLECTION_VIEW_PADDING);
        about.backgroundColor = [UIColor clearColor];
        [about setTitle:@"About" forState:UIControlStateNormal];
        [about setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        about.titleLabel.font = [UIFont fontWithName:@"GillSans-Light" size:18.0f];
...
- (void)reloadViewWithArray:(NSArray *)array {
    NSLog(@"Reloading menu...");
    self.menuItems = array;
    NSTimer *timer = [NSTimer timerWithTimeInterval:0.1 target:self selector:@selector(reloadCollectionView) userInfo:nil repeats:NO];
    [timer fire];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.title.text = [[menuItems objectAtIndex:indexPath.row] categoryName];
    return cell;
}
